...
function MakePublicTag($atts=array(),$refObj='',$fields=array())
{
	...
	****************************************/
	...
	$atts_fle = explode('_',$atts['tagname']);
	$plusfile = ZCMS_ROOT.'/zcms/'.$atts_fle[0].'/tpllib/'.$atts['tagname'].'.php';
...
class DedeTemplate
{
	...
	var $refObj = '';
	var $makeLoop = 0;
	var $tagStartWord =  '{zhuayi:';
	var $fullTagEndWord =  '{/zhuayi:';
	var $sTagEndWord = '/}';
	var $tagEndWord = '}';
	var $tpCfgs = array();
	...
	function __construct($templatedir='',$refDir='')
	{
		...
		if($templatedir=='')
		{
			$this->templateDir = ZCMS_ROOT.'/templates';
		}
		else
		{
		...
		$this->cacheDir = ZCMS_ROOT.'/data/tpl_cache/';//DEDEROOT.$GLOBALS['cfg_tplcache_dir'];
	...
	//设置标记风格
	function SetTagStyle($ts='{zhuayi:',$ftend='{/zhuayi:',$stend='/}',$tend='}')
	{
	...
	function LoadTemplate($tmpfile)
	{
		if(!file_exists($tmpfile))
		{
			...
			exit;
			//$error = array('error'=>"$tmpfile 模版路径错误！");
			//error($error);
		}
	...
	function SaveTo($savefile)
	{
		...
		{
			$str2=explode('/',$savefile);
			$savefile = str_replace($str2[(count($str2)-1)],'',$savefile);
		}
	...
	function CompilerOneTag(&$cTag)
	{
		...
		{
			$cTag->tagValue = $this->CompilerArrayVar('echo',$varname);
		...
		else if( $tagname=='pagelist' )
		{
			...
			$rsvalue = '<'.'?php'."\r\n".'$atts = array();'."\r\n".$rsvalue;
			$rsvalue .= ' echo  MakePublicTag($atts,$this->refObj,$fields); '."\r\n".'?'.">\r\n";
			$cTag->tagValue = $rsvalue;
		}
		else
		{
	...
	function CompilerInclude($filename, $isload=true)
	{
		...
		$okfile = ZCMS_ROOT.$filename;
		//echo $this->refDir."../".$filename;
		//exit;
		
		if($okfile=='') return 0;
		if( !$isload ) return 1;
		$itpl = new DedeTemplate();
